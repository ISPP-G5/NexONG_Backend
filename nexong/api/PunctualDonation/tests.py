...
class AdminPuntualDonationApiViewSetTestCase(TestCase):
    def setUp(self):
        ...
        self.puntualdonation = PunctualDonation.objects.create(
            ...
            proof_of_payment_document = SimpleUploadedFile("proof_of_payment_document.pdf", file_content),
            date= "2024-01-22",
        )

    def test_get_puntual_donations_by_admin(self):
        response = self.client.get(
            f"/api/punctual-donation/{self.puntualdonation.id}/", HTTP_AUTHORIZATION=f"Token {self.token.key}"
        )
